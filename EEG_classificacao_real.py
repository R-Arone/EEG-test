...
df = pd.read_csv('OpenBCI-RAW-penis.txt', sep=',', header = None)
df.drop(df.columns[[0,9,10,11,12]], axis=1, inplace=True)
df2 = pd.read_csv('cubo_magico_2_vezes.txt', sep=',', header = None)
...
valores2 = df2.values[1:-1]
filtred_signal_transpose2 = np.transpose(valores2)
...
hiddenlayer = 1
score = []
def train(hiddenlayer,hiddenlayer2,matrix_train,matrix_train2):
    MultiLayerPerceptron = MLPClassifier(solver='adam', alpha=1e-5, hidden_layer_sizes=(hiddenlayer,),
                                                  random_state=1, max_iter=100000,activation = 'logistic')
    ...
    X_train, X_test, y_train, y_test = train_test_split(np.transpose(matrix_train), y, test_size=.4)
...
for j in range (1,10):
    for i in range(1,200):
        score,cm = train(i,j,matrix_train,matrix_train2)
        scoret.append(score)
print(score)
print(cm)
plt.figure()
